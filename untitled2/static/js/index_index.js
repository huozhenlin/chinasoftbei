...
function getQueue() {
    $('#show').html('');
    var url = hostname + "/spliderqueueshow";
    //异步方式请求json
...
function getNum() {
    ...
    $.getJSON(url, function (res) {
        ...
        $('#splider_num').append(
            ...
            "<h5><span>爬虫队列中，共有定时爬虫</span>" + result.length + "<span>个</span></h5>"
        )
    })
};
...
function getSplider() {
    ...
    // 就绪爬虫url
    var url1 = hostname + '/jsonall?status=0';
    //历史爬虫url
    var url2 = hostname + '/jsonall?status=1';
    url_list = [url1, url2];
    $('#show').html('');
    // 正在进行的爬虫
...
function innerQueueHtml(id) {
    var url = hostname + "/querybyid?id=" + id;
    // alert(url);
    $.ajax({
        type: "get",
        url: url,
